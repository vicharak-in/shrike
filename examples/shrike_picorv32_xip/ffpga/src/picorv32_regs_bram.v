...
module picorv32_regs_bram (
    ...
    // SHRIKE PATCH (P20): x0 is STORAGE, not logic. The SLG47910 BRAM powers up
    // all-zero after configuration (established on hardware), and
    // entry 0 is never written: the core gates wen with latched_rd != 0
    // (picorv32.v: .wen(resetn && cpuregs_write && latched_rd)). So reads of
    // x0 return the stored zero and the former 32-bit zero-masks
    // ((raddr==0) ? 0 : rd) are deleted.
    assign rdata1 = rd;
    // rdata2 is unused with ENABLE_REGS_DUALPORT=0 (picorv32 takes rs2 from
    // the sequenced rs1 read); kept for interface completeness.
    assign rdata2 = rd;
