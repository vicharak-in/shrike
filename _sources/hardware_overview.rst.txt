...
=====================================
Shrike is a family low-cost, low-power, and easy-to-use FPGA development board that combines both the 
Renesas FPGA and microcontroller. It is designed for hobbyists, students, and professionals to explore and prototype FPGA-based designs with ease. 
...
The ESP32S3's LED is connected to GPIO 21 and the FPGA LED is connected to GPIO 16.

The LED's are active high meaning that when the GPIO pin is set to high the LED will turn on and when the GPIO pin is set to low the LED will turn off.

FLASH
######
The Shrike Dev Board features a 32Mb/4MB QSPI based Flash memory which is connected to ESP32S3 and . The part number for which is W25Q32JV. This flash is used to store the fpga bitstream and firmware. 
...
Both the IC on the board have separate programming models. The ESP32S3 
can be programmed using MicroPython, Arduino or ESP-SDK whereas the FPGA needed to be programmed using Verilog in the Renesas Go Configure hub.
